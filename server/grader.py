...
def grade_prediction(prediction: Any, example: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if predicted_action in ("delete", "escalate") or predicted_label in ("hate", "violence"): predicted_severity = "high"
    elif predicted_action == "flag" or predicted_label == "spam": predicted_severity = "medium"
    ...
    penalty = 0.0
    if predicted_action not in example["severity_actions"]: penalty -= WRONG_ACTION_PENALTY
